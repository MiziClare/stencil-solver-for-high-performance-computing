...
int *read_dims(char *, int);
int read_num_dims(char *);
...
int main(int argc, char *argv[]) {
    ...
    int c_num_dims = read_num_dims(check_filename);
    int *c_dims = read_dims(check_filename, c_num_dims);

    if(c_dims == NULL) {
        return -1;
    }

    int r_num_dims = read_num_dims(ref_filename);
    int *r_dims = read_dims(ref_filename, r_num_dims);
    if(r_dims== NULL) {
        return -1;
    }
